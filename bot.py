...
from user_profile import (
    ...
    set_premium_duration, get_premium_remaining_days, check_premium_expired,
    get_premium_status_text,
)
from support_handler import (
    start_support_session, handle_support_choice, confirm_support,
    get_tier_info, end_support_session, get_premium_benefits_text,
    is_in_support_session, get_support_session_state, support_sessions,
)
...
async def menu(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await safe_reply(update,
        ...
        "💖 /support — Support developer & dapatkan Premium!\n\n"
        "*Percakapan Bebas:*\n"
...
async def premium_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_premium(user_id):
        await safe_reply(update,
            "💎 *Kamu sudah Premium!*\n\n"
            f"{get_premium_status_text(user_id)}\n\n"
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_in_support_session(user_id):
        await safe_reply(
            update,
            "Foto diterima 👍\nKalau ini bukan bukti support, lanjut chat seperti biasa ya.",
        )
        return

    if get_support_session_state(user_id) != "awaiting_payment":
        await safe_reply(update, "Ketik /support untuk mulai support developer.")
        return

    tier = get_tier_info(user_id)
    if not tier:
        end_support_session(user_id)
        await safe_reply(update, "Sesi support tidak valid. Ketik /support untuk mulai ulang.")
        return

    set_premium_duration(user_id, tier.get("days", 1))
    confirmation = confirm_support(user_id, name) or "Terima kasih atas support kamu!"
    end_support_session(user_id)
    await safe_reply(
        update,
        confirmation + "\n\n✅ Bukti pembayaran diterima. Premium kamu sudah diaktifkan!",
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_in_support_session(user_id):
        ...
        if state == "viewing_menu":
            ...
            if ok:
                tier_idx = support_sessions.get(user_id, {}).get("tier_index", 0)
                response += "\n\n" + get_premium_benefits_text(tier_idx)
            await safe_reply(update, response)
            return
